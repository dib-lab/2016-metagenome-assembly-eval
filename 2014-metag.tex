...
We next used the fastq quality filter (@CTB what program exactly?)
from the FASTX-Toolkit to remove sequences with (@CTB describe what
those parameters do) using the parameters -Q33 -q 30 -p 50 (cite
fastx, give version).

\subsection*{Mapping}

We aligned all quality-filtered reads to the reference metagenome with bwa
aln and bwa samse (XXX give version and any command line options that
aren't default).  We aligned paired-end and orphaned reads separately,
and we aligned each as single reads using bwa samse.  We then used
samtools (version) to convert SAM files to BAM files for both
paired-end and orphaned reads.

We found chimeric reads with the bwa mem aligner with the default
parameters (XXX give bwa version).

\subsection*{Digital Normalization} 

We applied the {\tt normalize-by-median.py} script from khmer vXXX to
execute abundance normalization (``digital normalization'', @cite
Brown 2012) on the data, retaining paired reads with -p and using a
...
high-abundance reads (estimated k-mer coverage >= 20) at low-abundance
k-mers (k-mer abundance <= 2 -- @CTB check these parameters) to remove
erroneous k-mers (@cite Zhang et al., 2014 PLoS One; Zhang et al.,
2015 (peerJ)).
...
We next applied partitioning to the data (cite Pell et al. 2012, Howe
et al., 2014).  We first eliminated high-abundance k-mers that join multiple
species bins using {\tt filter-below-abund.py} script from khmer vXXX.
...
We assembled the reads using four different assemblers; Velvet \cite{velvet}, Idba \cite{idba}, Spades \cite{spades}, and Megahit \cite{megahit}.
(@CTB please check and use appropriate capitalization - look at the original
papers to see what they use. I think it's IDBA, MEGAHIT, SPADes.)
...
used \-exp\_cov auto \-cov\_cutoff auto.

For Idba \cite{idba},  we also used  --pre\_correction and -r for the pe files.
...
Quast vXXX \cite{quast} using quast.py with these parameters -R for the
...
To evaluate the initial reads, we mapped quality filtered reads to the
reference metagenome using bwa aln. We then extracted the unaligned
sequences (@CTB how? put in Methods; then remove this whole sentence).
...
mapping was high, with 92.0m reads having a MAPQ $\geq 30$.  (@CTB put
this ``Having almost all reads aligned to the reference, shows that
...
read spans two different genes (@CTB what does this mean? Do you have
a reference for this?). We used sam tools to convert the mapped sam
file to a bam file. Then we search for ’SA’ in the bam file, we have
310,131 chimeric reads using quality filtered reads.
...
total, only 3.01\% of the reads were removed (XXX reads, YYY bp),
...
% @CTB please format and shorten this subsection as above, and spell
% check :).
After digital normalization, the pair ended file contains
1687.59 megabases and 16,853,716 sequences while the single ended file
contains 5.86 megabases and 64,638 sequences.  After partitioning, we
got 29 partitions. The largest partition in paired-end files has
1379.27 megabases while the smallest partition has 7.14 megabases. The
total base pairs in all pair-end files partitions is 1651.53
megabases.  For single-end files, the largest partition has 13.90
megabases while the smallest partition has 2.52 megabases and in total
24.6 megabases.  Knowing that quality filtered reads has 10547.79
megabases for paired-end file and 184.44 megabases for single-end file
(see above for more details), clearly, digital normalizatation and
partitioning decrease the total number of reads.

We used bwa aln \cite{bwa-mem} to map digital normalized reads and
...
coverage. Digital normalized reads and partitioned reads represent
202,201,168 and 201,193,779 of the reference genome (205,603,715)
which represent 98.34\%, and 97.85\% of the reference respectively.

We estimated quality of mapping using MAPQ field in sam files, we have
15,248,657 and 15,164,099 with MAPQ $\geq 30$, using digital
normalized reads, and partitioned reads respectively.

We used bwa mem to search for chimeric reads by mapping digital
normalized reads, and partitioned reads to the reference genome. Then
we search for 'SA' in bam files, we have 28,969 and 26,960 chimeric
...
10,686,421 bp using quality filtered reads for Velvet, IDBA, SPAdes,
...
90.358\% using quality filtered reads for Velvet, IDBA, SPAdes, and
Megahit respectively.  Using digital normalization, the genome
fraction is 89.043\%, 91.003\%, 90.173\%, and 89.92\% for Velvet,
IDBA, SPAdes, and Megahit respectively.  Using partitioning, the
genome fraction is 88.879\%, 90.082\%, 89.272\%, and 88.769\% for
Velvet, IDBA, SPAdes, and Megahit respectively.  Clearly, digital
...
and $\sim 4$ hours using partitioning.  For IDBA assemblies, it took
...
applying digital normalization and partitioning respectively. For IDBA
...
conclude that Digitial normalization and partitioning treatments
