...
% http://online.liebertpub.com/doi/abs/10.1089/cmb.2017.0008 viral assembly
% update khmer ref to joss pub.
...
MEGAHIT and metaSPAdes assemblers \cite{Li2016,nurk2017metaspades}.
Importantly, the authors of the MEGAHIT benchmarking paper noted the
presence of unexpected sequence in this data set.
...
metagenome assemblers on several simulated data sets of varying
complexity, evaluating recovery of novel genomes and multiple strain
variants \cite{CAMI}.  Notably, CAMI concluded that ``The resolution
of strain-level diversity represents a substantial challenge to all
...
assembler.  Also see \cite{Greenwald2017} for an analysis of a
previously generated HMP benchmark data set; however, the Illumina
reads used for this study are much shorter than current sequencing and
are arguably not relevant to future studies.
...
data set.  This data set is the most complex synthetic community for
which bulk sequencing data is available, and has been used for several
independent assembly benchmarking efforts
\cite{Seah2015,Li2016,nurk2017metaspades}.  However, while previous
efforts have noted the presence of unexpected sequence data in the
data set \cite{Li2016}, no further analysis has been done to
characterize this sequence or its likely origins.

Below, we first update the list of reference genomes for Shakya et
al. to include the latest GenBank assemblies along with plasmids. We
then compare IDBA \cite{idba}, MetaSPAdes \cite{metaspades}, and
MEGAHIT \cite{megahit} performance on assembling this short-read data
set, and explore concordance in recovery between the three assemblers.
We describe the effects of ``strain confusion'' between multiple
strains.  We also detect and analyze several previously unreported
strains and genomes in the Shakya et al. data set.  We find that in
the absence of closely related genomes, all three metagenome
...
completely present in the reads.  Our report looks specifically at the
most poorly recovered genoems, provides strong guidance on choice of
assemblers, and extends previous analyses of this low-complexity
...
read data, we used {\tt bwa aln} (v0.7.7.r441) to map paired-end and
orphaned reads to the
...
Starting from the reference alignment calculated with bwa aln above,
we used samtools (v0.1.19) \cite{sam-stools} to convert SAM files to BAM
files for both paired-end and orphaned reads. To count the unaligned
reads, we included only those records with the ``4'' flag in the SAM
files \cite{sam-stools}.
...
\caption{\label{fig:coverage-profile} Cumulative coverage profile for the reference metagenome, based on read mapping. The x axis shows the number of reads that overlap each base, and the y axis shows the number of bases with that number of overlapping reads.}
\end{figure}
...
\begin{table}[t]
\caption{Jaccard containment of the reference in the reads. For each k-mer size we calculated the fraction of k-mers in the reference that are present in the reads.}
\centering
\begin{tabular}{|c|c|}
\hline
...
\begin{table}[!h]
\centering
\caption{Top uncovered genomes.  More than 5\% of the bases in four of the 64 reference genomes had no quality filetered reads mapping to them.}
...
\begin{table}[t]
  \caption{Genomes removed from reference for low 51-mer presence. The reads were missing 1\% of more the true 51-mers in thirteen of
  the 64 reference genomes. }
\centering
\begin{tabular}{|c|l|}
\hline
...
%Numbers in this section are up to date 22 december 2016
 ...
 %------------------------------------Cost of Assembly ---------------
 \begin{table}[h]
\caption{Running time and memory utilization of each of the assemblers. We measured the total CPU time (which includes individual processor compute), }
\centering
...
identity threshold, each of the three assemblies recovers more than 83.4\% of the
...
assemblies (right panel, Figure~\ref{fig:nga50}).  We found that 13 of
the 51 genomes were missing 5\% or more of bases in at least one
assembly, despite all 51 genomes having 99\% or higher read- and
51-mer coverage.  While some of these missing bases may be in the assembled
contigs that are less than 500 bp in length, contigs shorter than 500 bp
are unlikely to contain more than half of a typical bacterial gene
\cite{xu2006average}.
...
\subsection*{Omissions in this study: binning and parameter sweeps}
...
Overall, assembly of this mock community performs well, with good
...
(e.g. 10X Genomics \cite{Moss125211}), HiC approaches
\cite{SmukowskiHeil150722}, and cell-sorting cells into ``minimetagenomes'' \cite{ji2017metasort} show promise but these remain untested on
