...
\subsection*{What we can't assemble}
To find out what limits assembling, we mapped quality filtered reads using bwa mem \cite{bwa-mem} to the reference genome. We then, extracted the unaligned sequences. There are 2,521,366 unaligned paired-reads which represents 2.37\% of the quality filtered reads. We also looked for chimeric reads where a read spans two different genes, we found 76,392 chimeric pairs. 
CHECK NUMBERS IF WE NEED TO RERUN MAPPING
...
Using partitioning, we got also zero unaligned contigs, with partial unaligned length 2,053,035, 2,071,046, 2,069,811 and XX for Velvet, IDBA, SPAdes, and Megahit respectively which represent 1.02\%, 1.04\%, 1.04\%, and XX \% of their corresponding assemblies length respectively. 

\subsection*{Assembly Accurracy}
...
\begin{figure} [h] 
\begin{center}  
\includegraphics[height=3.2in,width=4.5in]{mincontig.pdf}  
...
\hline
\textbf{Percentage of unaligned}&31.45\%&33.92\%&37.99\%\\ [1ex]
\hline
\multicolumn{4}{|c|}{ \textbf{(2) Idba}}    \\ [0.5ex] % inserts table %heading
\hline
