...
To find what exists in the reference genome and is missed by the assemblies, we aligned the reference genome to each assembly. 
Using quality filtering, we got zero unaligned contigs, in addition to 29,776,904, 2,062,384, 2,063,510, and 2,063,473 partially unaligned length for Velvet, IDBA-UD, SPAdes, and MEGAHIT respectively which represent 14.48\%, 1.003\%, 1.003\%, and 1.003\% of the reference genome. 
...
In conclusion, $\sim$ 1.01\% of the reference genome is missed by the assemblies, except for Velvet/QC, which missed 14.48\%. of the reference. 


\subsection*{The de novo assemblies recover unexpected genomic sequence from the mock community}

We aligned the unaligned contigs of each assembly to the unaligned contigs of IDBA-UD quality filtered assembly. 
...
Having almost all reads aligned to the reference, shows that the data has a high quality. The high quality of the data is one reason the assemblers work the same (see below). However, understanding the behaviors of assemblers on less quality data is essential to decide which assembler is best to use. Analyzing the behaviors of assemblers on different datasets, using different quality and signatures is left for our future work.  
 ...
 Misassembled length is less than 1.3\% for all assemblies. We conclude that assembly works well although there are some rooms for improvements including enhancing accuracy, and decreasing time and memory requirements. Velvet shows the least performance in terms of accuracy and time, and memory utilizations. However, the assemblers work the same.
...
We conclude that digital normalization and partitioning are beneficial steps for assembly to reduce time and memory utilities while not effecting quality.   
